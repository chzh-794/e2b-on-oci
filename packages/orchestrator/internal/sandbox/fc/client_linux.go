...
func (c *apiClient) setMachineConfig(
	...
	// HugePages is only supported in Firecracker v1.6.0+
	// v1.5.0 API rejects the field, so we skip it
	if hugePages {
		// Only set if Firecracker version >= 1.6.0 (check via API version or skip for v1.5.0)
		// For now, skip HugePages for v1.5.0 compatibility
