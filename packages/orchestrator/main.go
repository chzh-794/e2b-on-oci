...
func run(port, proxyPort uint) (success bool) {
	...
	lockPath := env.GetEnv("ORCHESTRATOR_LOCK_PATH", fileLockName)
	...
	if !env.IsDevelopment() && !forceStop {
		...
		if err != nil {
			log.Fatalf("Failed to create lock file %s: %v", lockPath, err)
		}
		defer func() {
			fileErr := f.Close()
			if fileErr != nil {
				log.Printf("Failed to close lock file %s: %v", lockPath, fileErr)
			}

			// Remove the lock file on graceful shutdown
			if success == true {
				if fileErr = os.Remove(lockPath); fileErr != nil {
					log.Printf("Failed to remove lock file %s: %v", lockPath, fileErr)
				}
			}
		}()
	}
