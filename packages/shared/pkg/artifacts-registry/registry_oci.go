...
func (o *OCIArtifactsRegistry) getAuth(ctx context.Context) (*authn.Basic, error) {
	...
	return nil, fmt.Errorf("failed to acquire OCIR auth token via instance principal and no OCIR_USERNAME/OCIR_PASSWORD provided")
}
...
func (o *OCIArtifactsRegistry) fetchInstancePrincipalToken(ctx context.Context) (*authn.Basic, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("unexpected status from OCIR token endpoint: %s", resp.Status)
	}
